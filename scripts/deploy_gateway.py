...
"""Deploy enma-gateway to AWS ECS — digest-pinned, rollout-verified.

The gateway twin of ``scripts/deploy.py``. Kept as a separate file (rather
than a parameterised single script) on purpose: the backend deploy path is
load-bearing and proven, so the gateway deploy must not be able to break it.
The two share the same digest-pin + rollout-verify discipline:

    build zip (git archive HEAD:enma-gateway) -> S3 -> CodeBuild ->
    read the freshly-pushed ECR digest -> register a task def pinned to
    @sha256:DIGEST -> force-new-deployment -> poll until the RUNNING task's
    image digest matches what we built.

Why digest-pinned: ``:latest`` + ``--force-new-deployment`` can silently
no-op on a Fargate cache (see deploy.py header). We only declare success
once the running task is on the new revision AND the expected digest.

Usage
-----
    python scripts/deploy_gateway.py [--build/--no-build] [--wait/--no-wait]
"""
...
ECR_REPO = "enma-gateway"
S3_SOURCE_BUCKET = "enma-build-source-603013471251"
S3_SOURCE_KEY = "enma-gateway.zip"
CODEBUILD_PROJECT = "enma-gateway-build"
GIT_SUBTREE = "HEAD:enma-gateway"
...
def build_zip(repo_root: Path, dest: Path) -> None:
    """``git archive`` the enma-gateway tree from HEAD into ``dest``."""
    subprocess.run(
        ["git", "archive", "--format=zip", GIT_SUBTREE, "-o", str(dest)],
        check=True,
        cwd=repo_root,
    )
...
def upload_to_s3(local: Path) -> None:
    aws(
        ["s3", "cp", str(local), f"s3://{S3_SOURCE_BUCKET}/{S3_SOURCE_KEY}"],
        capture=False,
    )


def start_codebuild() -> str:
    result = aws_json(["codebuild", "start-build", "--project-name", CODEBUILD_PROJECT])
...
def wait_for_codebuild(build_id: str) -> None:
    ...
    while True:
        result = aws_json(["codebuild", "batch-get-builds", "--ids", build_id])
        assert isinstance(result, dict)
...
def register_pinned_task_def(image_uri_with_digest: str) -> str:
    """Clone the current task def, pin the image to the digest URI, register."""
    td = current_task_def()
    for key in (
        "taskDefinitionArn",
        "revision",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Deploy enma-gateway to ECS.")
    parser.add_argument(
        "--build",
    ...
    if args.build:
        ...
        zip_path = root / "enma-gateway-deploy.zip"
        if zip_path.exists():
            zip_path.unlink()
