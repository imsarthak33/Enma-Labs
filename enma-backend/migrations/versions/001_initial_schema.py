...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("telegram_bot_token", sa.Text(), nullable=False, comment="Encrypted at rest — never log or expose."),
        sa.Column("subscription_tier", sa.String(50), server_default=sa.text("'starter'"), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("role", sa.String(20), server_default=sa.text("'member'"), nullable=False, comment="admin | partner | member"),
    ...
    op.create_table(
        ...
        sa.Column("gstin", sa.String(15), nullable=True, comment="15-char GSTIN. Validated at app layer via regex + checksum."),
    ...
    op.create_table(
        ...
        sa.Column("document_type", sa.String(50), nullable=False, comment="B2B_INVOICE | B2C_INVOICE | FREIGHT | RESTAURANT | IMPORT | PROFESSIONAL | CAPITAL_GOODS"),
        sa.Column("source_file_ids", postgresql.JSONB(), nullable=False, comment="Telegram file_id array."),
        sa.Column("extraction_data", postgresql.JSONB(), nullable=False, comment="Full structured extraction output."),
        sa.Column("tax_verdict", postgresql.JSONB(), nullable=True, comment="Five optimisation variables: claim, defer, block, rcm, tds."),
        sa.Column("verification_result", postgresql.JSONB(), nullable=True, comment="Red-team deterministic verification output."),
        sa.Column("filing_period_month", sa.Integer(), nullable=True),
        sa.Column("filing_period_year", sa.Integer(), nullable=True),
        sa.Column("processing_status", sa.String(20), server_default=sa.text("'pending'"), nullable=False, comment="pending | processing | completed | failed"),
    ...
    op.create_index("idx_documents_filing", "documents", ["ca_firm_id", "filing_period_year", "filing_period_month"])
    ...
    op.create_table(
        ...
        sa.Column("id", postgresql.UUID(as_uuid=True), server_default=sa.text("uuid_generate_v4()"), nullable=False),
        sa.Column("ca_firm_id", postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column("client_id", postgresql.UUID(as_uuid=True), nullable=True, comment="NULL = firm-wide rule; set = client-specific."),
        sa.Column("rule_type", sa.String(50), nullable=False, comment="correction | preference | exception | procedure"),
        sa.Column("rule_text", sa.Text(), nullable=False, comment="Human-readable rule description."),
        sa.Column("source", sa.String(50), nullable=False, comment="human_correction | manual_entry | system_learned"),
        sa.Column("is_active", sa.Boolean(), server_default=sa.text("TRUE"), nullable=False),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.text("NOW()"), nullable=False),
        sa.Column("updated_at", sa.DateTime(timezone=True), server_default=sa.text("NOW()"), nullable=False),
        sa.PrimaryKeyConstraint("id"),
    ...
    op.create_table(
        ...
        sa.Column("ca_firm_id", postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column("chat_id", sa.BigInteger(), nullable=False),
        sa.Column("client_id", postgresql.UUID(as_uuid=True), nullable=True, comment="NULL if no client context active."),
        sa.Column("role", sa.String(20), nullable=False, comment="user | assistant | system"),
        sa.Column("content", sa.Text(), nullable=False),
        sa.Column("message_id", sa.BigInteger(), nullable=True, comment="Telegram message_id — NULL for system-generated turns."),
    ...
    op.create_index("idx_conversations_lookup", "conversations", ["ca_firm_id", "chat_id", "created_at"])
    ...
    op.create_table(
        ...
        sa.Column("status", sa.String(20), server_default=sa.text("'pending'"), nullable=False, comment="pending | in_progress | completed | cancelled"),
    ...
    op.create_table(
        ...
        sa.Column("filing_snapshot", postgresql.JSONB(), nullable=False, comment="Full state at time of approval — document IDs, totals, etc."),
        sa.Column("approval_hash", sa.Text(), nullable=False, comment="SHA-256 of the filing_snapshot for integrity verification."),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.text("NOW()"), nullable=False),
        sa.PrimaryKeyConstraint("id"),
        sa.ForeignKeyConstraint(["ca_firm_id"], ["ca_firms.id"], ondelete="CASCADE"),
        sa.UniqueConstraint("ca_firm_id", "filing_month", "filing_year", name="uq_filing_approvals_firm_period"),
    )
    ...
    op.create_table(
        ...
        sa.Column("payload_hash", sa.Text(), nullable=False, comment="SHA-256 of the base64-decoded JSON envelope."),
    ...
    op.create_table(
        ...
        sa.Column("stage_reached", sa.Integer(), nullable=False, comment="1=session, 2=caption, 3=gstin, 4=vendor, 5=explicit_ask"),
    ...
    op.create_table(
        ...
        sa.Column("ca_firm_id", postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column("client_id", postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column("notification_type", sa.String(50), nullable=False, comment="document_chase | deadline_reminder | filing_reminder"),
    ...
    op.create_index("idx_notifications_cooldown", "client_notifications", ["ca_firm_id", "client_id", "notification_type", "cooldown_until"])
