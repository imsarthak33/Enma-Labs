...
_SUPERVISOR_TASK: Final[str] = (
    ...
    "the dominant bucket.\n\n"
    "RECENT CONTEXT — USE THE CONVERSATION HISTORY\n"
    "The conversation history above includes the bot's prior turns "
    "verbatim, including any 'Document processed' summaries (full "
    "extraction, verdict, totals) the bot sent the CA in this thread. "
    "When the CA says 'this invoice', 'the one we just saw', 'the "
    "most recent invoice', or 'invoice 128' WITHOUT a specific Ref "
    "hash, FIRST scan the history for a matching summary and answer "
    "from it. Only call query_document_by_ref or query_documents if "
    "the history has nothing relevant. A locked-period refusal "
    "('Not added to books') still leaves the extraction in history — "
    "the LLM CAN answer 'what was the grand total' or 'is the ITC "
    "claimable' from the cached summary even when the doc is not "
    "persisted. Example:\n"
    "  History (prior turn): 'Document processed — Invoice 128 from "
    "CLEIND, Grand total 15680.01, taxable 13288.14, IGST 2391.87. "
    "Not added: August 2025 is locked.'\n"
    "  User now: 'is invoice 128 ITC claimable?'\n"
    "  WRONG: call query_document_by_ref → not found → 'I couldn't "
    "find invoice 128.'\n"
    "  RIGHT: 'Invoice 128 wasn't added to the books — August 2025 "
    "is locked. Based on the extraction I showed earlier (IGST "
    "₹2391.87, vendor CLEIND), it would have been claimable as ITC, "
    "but the locked filing means it can't be claimed in the August "
    "return. To use the credit, unlock the filing or carry it to a "
    "future return.'\n"
)
