...
def verify_multi_rate_tax(extraction: InvoiceExtraction) -> VerificationResult:
    ...
    computed_total_cgst = sum((b["cgst"] for b in computed_by_rate.values()), Decimal("0"))
    computed_total_sgst = sum((b["sgst"] for b in computed_by_rate.values()), Decimal("0"))
    computed_total_igst = sum((b["igst"] for b in computed_by_rate.values()), Decimal("0"))
    ...
    stated_total_cgst = sum((slab.cgst for slab in extraction.tax_breakdown), Decimal("0"))
    stated_total_sgst = sum((slab.sgst for slab in extraction.tax_breakdown), Decimal("0"))
    stated_total_igst = sum((slab.igst for slab in extraction.tax_breakdown), Decimal("0"))
